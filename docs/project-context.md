...
> ⚠️ **HISTORICAL — old model.** This describes the original HTML-upload prototype
> (branded "LayerSync"). The project has since pivoted to the **Git-based** model
> in [`iterait-build-flow.md`](./iterait-build-flow.md), which is the current
> source of truth. Kept for reference only. The pre-pivot app is archived at the
> `pre-git-pivot` git tag.
